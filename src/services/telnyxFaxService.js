  ...
  try {
    ...
    const response = await fetch(`${TELNYX_API_URL}/faxes`, {
      ...
      body: JSON.stringify({
        ...
        quality: 'high',
        header_info: { header_enabled: false },
        metadata: {
...
export async function listReceivedFaxes(params = {}) {
  const { limit = 20, dateFrom = null, dateTo = null } = params;
  if (!TELNYX_API_KEY) {
    return [];
  }
  try {
...
export async function sendBatchFaxes(faxList) {
  ...
  for (const fax of faxList) {
    try {
      ...
      results.push({ ...fax, ...result, success: true });
      await new Promise(resolve => setTimeout(resolve, 1000));
    } catch (error) {
      results.push({ ...fax, success: false, error: error.message });
    }
  }
  return results;
}
