...
// MAIN COMPONENT
// ================================================================================
const CreditScoreOptimizer = ({ clientId, onClose }) => {
  // ===== STATE MANAGEMENT =====
    ...
    try {
      ...
      const revolvingAccounts = tradelines
        .filter(t => {
          ...
          return accountType.includes('revolving') || 
        ...
        .map(tradeline => ({
          ...
          accountAge: tradeline.dateOpened ? 
            Math.floor((Date.now() - new Date(tradeline.dateOpened).getTime()) / (1000 * 60 * 60 * 24 * 365)) : 0,
          isRevolving: true,
          source: 'idiq',
      ...
      console.error('Error importing IDIQ report:', error);
                      ...
                      <Select value={idiqReportId} onChange={(e) => setIdiqReportId(e.target.value)} label="Select Report">
                        {availableReports.map(report => (
                          <MenuItem key={report.id} value={report.id}>
                            {new Date(report.createdAt?.toDate()).toLocaleDateString()} - {report.status || 'Complete'}
                          </MenuItem>
                        ))}
                      </Select>
                    </FormControl>
