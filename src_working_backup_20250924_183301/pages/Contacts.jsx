    ...
    if (searchTerm) {
      filtered = filtered.filter(contact => {
        ...
        return (
          ...
          contact.name?.toLowerCase().includes(searchLower) // Added name field
        );
      });
    }
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 mb-6">
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              >
                Leads ({contacts.filter(c => (c.type || c.category) === 'lead').length})
              </button>
              <button
              ...
              >
                Clients ({contacts.filter(c => (c.type || c.category) === 'client').length})
              </button>
              <button
              ...
              >
                Affiliates ({contacts.filter(c => (c.type || c.category) === 'affiliate').length})
              </button>
            </div>
          </div>
        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {contact.leadScore ? (
                        <div className="flex items-center gap-2">
                          <div className={`w-3 h-3 rounded-full ${scoreColors[Math.min(6, Math.max(1, Math.round(contact.leadScore * 0.6)))]}`} />
