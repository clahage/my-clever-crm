...
const chartOptions = (dynamicColor, darkMode, legendFilter, annotationConfig, onChartClick) => ({
  responsive: true,
  ...
  plugins: {
    legend: {
      ...
      onClick: (e, legendItem, legend) => {
        if (legendFilter) legendFilter(legendItem.datasetIndex);
      },
    },
    title: { display: false },
    tooltip: {
      ...
      borderWidth: 2,
      callbacks: {
        label: ctx => `Value: ${ctx.parsed.y ?? ctx.parsed}`,
        footer: ctx => `Date: ${ctx[0]?.label ?? ''}`,
      },
  ...
  elements: {
    line: { borderColor: dynamicColor },
    point: { backgroundColor: dynamicColor },
  },
          ...
          <div className="h-64" id="revenue-chart">
            {chartType === 'bar' ? (
              <Bar
                ...
                options={chartOptions(dynamicColor, darkMode, legendFilter('revenue'), annotationConfig, (e, els) => handleChartClick(e, els, 'revenue'))}
              />
            ) : (
              <Line
                ref={revenueChartRef}
        ...
        >
          ...
          <div className="h-64" id="accuracy-chart">
            <Bar
              ...
              options={chartOptions(dynamicColor, darkMode, legendFilter('accuracy'), annotationConfig, (e, els) => handleChartClick(e, els, 'accuracy'))}
            />
          </div>
        </motion.div>
          ...
          <div className="h-64" id="conversion-chart">
            {chartType === 'bar' ? (
              <Bar
                ...
                options={chartOptions(dynamicColor, darkMode, legendFilter('conversion'), annotationConfig, (e, els) => handleChartClick(e, els, 'conversion'))}
              />
            ) : (
              <Line
                ref={conversionChartRef}
        ...
        >
          ...
          <div className="h-64 flex items-center justify-center" id="leadsource-chart">
            <Pie
              ...
              options={chartOptions(dynamicColor, darkMode, legendFilter('leadsource'), annotationConfig, (e, els) => handleChartClick(e, els, 'leadsource'))}
            />
          </div>
        </motion.div>
      </div>
